...
Structural rewriting of the current type codec and explicit scoped forwarding are delivered. Next migrate all importer/specializer consumers to lossless type/member/signature nodes, including optional modifiers, calling conventions and function pointers. Exact application closure from explicit directories, canonical input locks, bounded file snapshots and cooperative cancellation checkpoints are now delivered; see [input contracts](input-locks.md). Remaining resolution work includes reviewed framework-facade expansion, richer loader policy, package-store integration and complete CPU/heap isolation. Preserve exact diagnostics instead of introducing fallback resolution.

The current gate covers relocated locked cyclic/forwarding graphs, content/version/reference-only conflicts and source/BCL/toolchain drift on both hosted targets. Remaining acceptance: multi-package graphs with lossless overload/modifier conflicts, deterministic output, no reference-stub execution and both-target differential coverage. Add portable-PDB source provenance and source maps separately from semantic identity.
...
Qualify Windows/macOS/ARM64/browser hosts and explicit capability adapters. Input-byte/file limits and canonical managed-input/toolchain locks are implemented, but they do not bound all compiler allocations or interrupt a long single operation. Add hostile-metadata fuzzing, process isolation, complete environment/source/license manifests and versioned ABI migration tests. Completion requires tests for both claimed targets, adjacent rejection cases and current documentation, not only a successful sample.
