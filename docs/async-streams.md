...
The separately implemented [host-clock-v1 contract](host-clocks.md) adds exact Int32 Task.Delay and timed CTS/CancelAfter overloads. Native callbacks publish readiness; translated managed scheduler code performs completion. Task and stream adapters wait on native notifications when idle, including awaited iterator cleanup. This does not add cross-thread or execution-context semantics.
