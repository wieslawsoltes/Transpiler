...
public static class SourceEmitter
{
    ...
    public static GeneratedSource Emit(CompilationAnalysis analysis, SourceTarget target)
    {
        ...
        output.AppendLine(python ? "# Generated by Transpiler. Profile: portable-mvp; metadata schema: 1." : "// Generated by Transpiler. Profile: portable-mvp; metadata schema: 1.");
        ...
        if (python)
        {
            ...
            output.AppendLine("R = CliRuntime(metadata)");
        }
        else
        {
            output.AppendLine("const metadata = " + json + ";");
            output.AppendLine("const R = new CliRuntime(metadata);");
        }
        var count = 0;
        foreach (var method in analysis.Methods)
        {
            EmitMethod(output, method, metadata, python);
    ...
    private static void EmitMethod(StringBuilder output, MethodAnalysis analysis, BackendMetadata metadata, bool python)
    {
        var method = analysis.Method;
        var nullValue = python ? "None" : "null";
        var argumentTypes = method.IsStatic ? method.Reference.Parameters : new[] { method.Reference.Type }.Concat(method.Reference.Parameters).ToArray();
        ...
        foreach (var instruction in method.Instructions.Where(i => analysis.StackBefore.ContainsKey(i.Offset)))
        {
            ...
            else if (op is "ldarg" or "ldloc") Push($"{(op == "ldarg" ? "a" : "l")}[{i.Operand}]");
            ...
            else if (op == "dup") Push(python ? "s[-1]" : "s[s.length - 1]");
            ...
            else if (op is "call" or "callvirt" or "newobj")
            {
                ...
                var expression = op == "newobj" ? $"R.new_object({Quote(metadata.MethodId(reference))}, z)" :
                    $"R.call({Quote(metadata.MethodId(reference))}, z, {Bool(op == "callvirt", python)})";
