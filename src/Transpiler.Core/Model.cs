...
{
    public string Key => $"{Type}::{Name}({string.Join(",", Parameters)})";
}
    ...
    MethodDefinitionModel[] Methods, FieldDefinitionModel[] Fields)
{
    ...
    public MethodDefinitionModel? Resolve(MethodReference method) => method.Assembly == Name
        ? Methods.FirstOrDefault(m => m.Key == method.Key && m.Reference.ReturnType == method.ReturnType) : null;
    public FieldDefinitionModel? Resolve(FieldReference field) => field.Assembly == Name
        ? Fields.FirstOrDefault(f => f.Reference.Key == field.Key) : null;
