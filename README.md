...
The generated `stream` adapter consumes direct interfaces, concrete/inherited/value-type enumerables and one Task/ValueTask factory wrapper. Object-erased and multi-interface results require an explicit linked element selection. `streamInfo` / `stream_info` exposes choices without executing application code. Acquisition, cursor and ValueTask ownership are translated C# with explicit reachability roots. Factory invocation is lazy and cleanup is single-consumption. See [generalized stream exports](docs/stream-exports.md) and the runnable [StreamFactories sample](samples/StreamFactories.cs).
...
Native async-iterator adapters use managed-stream-v2: concrete sources and single Task/ValueTask factories are supported; erased object results use explicit `elementType` / `element_type` selection and a checked managed cast. Nested awaitables, dynamic generic construction and general object serialization remain unsupported. `runtimeInfo`/`runtime_info` reports `activeStreams`, `activeStreamFactories` and `streamPolicy`; pending factory/move/disposal cleanup remains active until real retirement. Pump-step budgets do not interrupt infinite managed code or a host hook that never resolves.
